package exercises.chapter01.FirstStepsWithZIO
...
object FirstStepsWithZIO {
  ...
  object Exercise5 {
    ...
    for {
      int <- random
      _ <- printLine("Giess a number from 1 to 3:")
      num <- readLine
      _ <- if num == int.toString then printLine("Ypu guessed right!")
  ...
  object Exercise7 {
    ...
    def collectAll[R, E, A](in: Iterable[ZIO[R, E, A]]): ZIO[R, E, List[A]] = {
      if in.isEmpty then succeed(List.empty)
